...
export class PowerPage implements OnInit {
  ...
  async ngOnInit() {
    ...
    const timeNow = '/chart/Time/'+dayFormat
    
    this.itemRef = this.db.object('/chart/Watt/Sunday');
    this.itemRef.snapshotChanges().subscribe(Sunday=> {

      this.itemRef = this.db.object('/chart/Watt/Monday');
    this.itemRef.snapshotChanges().subscribe(Monday => {

      this.itemRef = this.db.object('/chart/Watt/Tuesday');
      this.itemRef.snapshotChanges().subscribe(Tuesday=> {

    this.itemRef = this.db.object('/chart/Watt/Wednesday');
    this.itemRef.snapshotChanges().subscribe(Wednesday=> {

      this.itemRef = this.db.object('/chart/Watt/Thursday');
      this.itemRef.snapshotChanges().subscribe(Thursday=> {

        this.itemRef = this.db.object('/chart/Watt/Friday');
        this.itemRef.snapshotChanges().subscribe(Friday=> {
          

          this.itemRef = this.db.object('/chart/Watt/Saturday');
     ...
     let total = dayOfWeek/1000
      ...
      const itemRef = this.db.object('wattUsage/total');
